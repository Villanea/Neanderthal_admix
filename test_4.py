...
def neanderthal_admixture_model(num_modern=1000,anc_pop = 1, anc_num = 1, anc_time=900,mix_time=1000,split_time=12000,f=0.03,Ne0=10000,Ne1=2500,mu=1.5e-8,rho=1.0e-8,length=10000000,window_size = 1000000,number_sites = 1,num_rep=100,coverage=False):
	...
	freq = []
	leng = []
	interval = [] #probs don't need to store the interval
	#window_size & number_sites
	#FYI mean fragment length from test_2 model ~6000 bp
	sim_num = 0	
	cur_start = 0
	cur_end = window_size-1
	windows = [i]*(length/window_size)
	cur_win = 1
	for sim in sims:
	...
	for line in range(0,len(leng)):
		...
		outfile.write('\n')
	outfile.close()
	return np.array(freq), np.array(leng)
