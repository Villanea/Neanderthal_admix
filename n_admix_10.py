...
par = 1 #ID number for parallelization
outfile = open('outfile_sim%s.bed' %(par), 'w+')
outfile.close()
		...
		for sim in sims:
			...
			for tree in sim.trees():
				...
				while cur_site >= F_int[0] and cur_site < F_int[1]:
					...
					print cur_site
		outfile = open('outfile_sim%s.bed' %(par), 'a')
...
#bedops
os.system("sort-bed outfile_sim%s.bed > outfile_sim%s_sorted.bed" %(par,par))
os.system("rm outfile_sim%s.bed" %(par))
os.system("bedops --element-of 1 outfile_sim%s_sorted.bed human_genome_mask_sorted.bed > outfile_sim%s_masked.bed" %(par,par))
os.system("rm outfile_sim%s_sorted.bed" %(par))
...
#sys_stat
#def symmetry_stat():
EU = np.genfromtxt('outfile_sim%s_masked.bed' %(par), usecols=3)
AS = np.genfromtxt('outfile_sim%s_masked.bed' %(par), usecols=4)

	#delete sim file
os.system("rm outfile_sim%s_masked.bed" %(par))
...
ID = par #plus other arguments once we loop through bootstrap
...
#def outmatrix():
outmatrix = open('symmetry_matrix%s' %(ID), 'w+')
